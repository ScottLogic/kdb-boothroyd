...
describe("Server Connection", () => {
  ...
  it("should not let me connect without details entered", async function () {
    ...
    assert.strictEqual(await connectButton.getAttribute("aria-disabled"), "true", "Connect button is not disabled")
  ...
  it("should connect to the server successfully", async function () {
    ...
    assert.notStrictEqual(connectButton, null, "Connect button does not exist")
    assert.strictEqual(await connectButton.getAttribute("aria-disabled"), null, "Connect button is disabled")

    await connectButton.click()
    await this.modal.waitForElementState("hidden")

    const tabs = await this.appWindow.$$(".connection-tabs button.ms-Pivot-link:visible")
    
    
    assert.strictEqual(tabs.length, 1)
    assert.strict(await tabs[0].getAttribute("aria-selected"), "true", "New connection tab is not selected")

    const tabText = await tabs[0].$('.ms-Pivot-text')
    assert.strictEqual(await tabText.innerText(), "Localhost", "Connection tab is labelled incorrectly")
